...
│       │   │       ├── settings/      # Settings
│       │   │       └── theme/         # Colors, Theme
│       │   └── sqldelight/            # org/sjbtimdan/linden/*.sq (schema v1, no migrations)
...
- `shared/src/commonMain/sqldelight/org/sjbtimdan/linden/` - SQLDelight `.sq` files (schema v1, no migrations)
...
|------------|--------|--------|
| Editing an `.sq` table requires deleting the local DB | Tech | Schema is v1 with no migrations; the persisted desktop DB at `~/.linden/linden.db` must be deleted after a DDL change |
