...
- **App language override** (`AppLanguage` in `model/AppLanguage.kt`, since 2026-09-06): Settings → Language stores a BCP-47 tag under `SettingsDao` `language` key ("system"/"en"/"it"/"zh-CN"/"zh-HK"; `SYSTEM` = follow platform, nothing written at startup; Traditional Chinese = Hong Kong only — TW is not a target market). `App` collects it and calls `ApplyLanguageOverride(language)` (expect/actual in `ui/AppLocale.*`) **during composition before the subtree**, wrapped in `key(language)` so everything re-composes under the new `Locale.setDefault`. Dates/money (both platforms) and string resources (desktop) follow automatically; on Android the app-context configuration-locale update is deferred until the first translated resources land (Phase 4). Picker labels: translated "System default" + language autonyms (never translate autonyms); keep option labels distinct from other "System" copy so UI tests stay unambiguous.
