...
### Decision
Store all money as integer minor units (`Long`), never `Double`/`BigDecimal`. `450` = 4.50. All currencies (CHF/CNY/EUR/GBP/HKD/INR/JPY/SGD/USD) use a 2-decimal minor unit. `amount` columns in `.sq` files are `INTEGER`.
